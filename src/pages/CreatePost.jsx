...
const CreatePost = () => {
  ...
  const handleSubmit = e => {
    ...
    if (postTitle.trim() === "") {
      setTitleError("The title must not be left blank.");
      hasError = true;
    }

    if (selectedValue === "option1") {
      if (pollOptions.length <= 1) {
        setOptionError("You must have a minimum of 2 options.");
        hasError = true;
      } else if (!pollOptions.every(option => option.name.trim() !== "")) {
        setOptionError("You must fill in all the fields for voting options.");
        hasError = true;
      }
    }
            ...
            <div className="mt-4">
              ...
              <div className="pb-4">
                ...
                <select
                  ...
                  className={`border-2 border-gray-500 p-2 rounded-md w-full focus:outline-none focus:border-indigo-700 focus:ring-indigo-700`}
                >
                  <option value="">Add-on not selected</option>
                  <option value="option1">Create voting</option>
                </select>
